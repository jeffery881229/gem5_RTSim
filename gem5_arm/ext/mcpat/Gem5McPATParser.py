...
def getConfValue(confStr):
    ...
    for x in spltConf:
        currHierarchy += x
        if currConf is None:
            logging.error("Configuration hierarchy '%s' is None at '%s'.", confStr, currHierarchy)
            raise KeyError(f"Key '{x}' not found in configuration at '{currHierarchy}'")
        if x.isdigit():
            currConf = currConf[int(x)]
        elif isinstance(currConf, dict) and x in currConf:
            currConf = currConf[x]
        else:
            logging.error("Key '%s' not found in configuration at '%s'.", x, currHierarchy)
            raise KeyError(f"Key '{x}' not found in configuration at '{currHierarchy}'")
        currHierarchy += "."
    logging.info("Value for '%s' resolved to: %s", confStr, currConf)
    return currConf
