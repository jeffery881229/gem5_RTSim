...
// #define Embe_PIM_ADDR 0xf00000001ULL      // virtual address of the Patch Embedding PIM
...
int main(){
    ...
    FILE *fp = fopen("/RAID2/LAB/css/cssRA01/gem5/data_set/CIFAR-10/cifar10_img0_128x128.bin", "rb");
    /*
    /RAID2/LAB/css/cssRA01/gem5/data_set/CIFAR-10/cifar10_img0_128x128.bin
    /RAID2/LAB/css/cssRA01/gem5/data_set/ImageNet_ILSVRC-2012/ILSVRC2012_val_00000024_128x128.bin
    /RAID2/LAB/css/cssRA01/gem5/data_set/Oxford_102_Flower/flower_img0_128x128.bin
    /RAID2/LAB/css/cssRA01/gem5/data_set/SVHN/svhn_img0_128x128.bin
    */
    if (!fp) {
...
*/
    ...
    *embedding_pim_addr = 0x12;
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
    // uint8_t *qkv_pim_addr = (uint8_t *)QKV_cal_PIM_ADDR;
    // uint8_t *temp_vir_addr3 = (uint8_t *)malloc(sizeof(uint8_t));
    // temp_vir_addr3[0] = *qkv_pim_addr;

    // number of heads = 2
    // some declarations
    uint8_t *q_kt_pim_addr = (uint8_t *)Q_KT_cal_PIM_ADDR;
    ...
    uint8_t *soft_v_cal_PIM = (uint8_t *)Soft_V_cal_PIM_ADDR;
    ...
    for(int k = 0; k < numHeads; k++) {
        ...
        *embedding_pim_addr = 0x13;
        printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
        // temp_vir_addr4[0] = *q_kt_pim_addr;
        ...
        *embedding_pim_addr = 0x14;
        printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
        // temp_vir_addr5[0] = *soft_v_cal_PIM;
    }

    // result * Wo + bo (PIM)
    *embedding_pim_addr = 0x15;
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
    // uint8_t *wo_bo_pim = (uint8_t *)Wo_bo_cal_PIM_ADDR;
    // uint8_t *temp_vir_addr6 = (uint8_t *)malloc(sizeof(uint8_t));
    // temp_vir_addr6[0] = *wo_bo_pim;

    // Residual + LayerNorm
    // 1. add result with original (PIM)
    *embedding_pim_addr = 0x16;
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
    // uint8_t *add_back_pim = (uint8_t *)Add_back_PIM_ADDR;
    // uint8_t *temp_vir_addr7 = (uint8_t *)malloc(sizeof(uint8_t));
    // temp_vir_addr7[0] = *add_back_pim;
    ...
    *embedding_pim_addr = 0x17;
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
    // uint8_t *feed_forward_pim = (uint8_t *)Feed_Forward_PIM_ADDR;
    // uint8_t *temp_vir_addr8 = (uint8_t *)malloc(sizeof(uint8_t));
    // temp_vir_addr8[0] = *feed_forward_pim;

    // residual + LayerNorm
    // 1. add back to original result
    *embedding_pim_addr = 0x18;
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)embedding_pim_addr, *embedding_pim_addr);
    // uint8_t *add_back_pim_2 = (uint8_t *)Add_back_PIM_ADDR_2;
    // uint8_t *temp_vir_addr9 = (uint8_t *)malloc(sizeof(uint8_t));
    // temp_vir_addr9[0] = *add_back_pim_2;
