...
// ---------------------- 追加或調整的巨集定義 ----------------------
#define PIM_ADDR 0xf0000000ULL      // virtual address of the PIM command
...
int main(){
    ...
    //****************************
    uint8_t *pim_addr = (uint8_t *)PIM_ADDR;
    *pim_addr = 0x10; // 在 RT-SIM 的 SubArray.cpp 寫入 0x10 代表執行 patch embedding
    printf("Wrote to addr: %p, value: 0x%02x\n", (void*)pim_addr, *pim_addr);
    ...
    *pim_addr = 0x12;
    ...
    // note: 有些計算沒辦法使用 PIM，例如浮點數運算，只好在這裡做處理
    // number of heads = 2
    // some declarations
    ...
    for(int k = 0; k < numHeads; k++) {
        ...
        *pim_addr = 0x13;
        ...
        // (因為上面已經用PIM計算 Q*KT 了，然後 Q*KT 的矩陣大小跟 W_patch_quant 一樣大，
        //  所以我就沒有再新宣告一個矩陣了，而是直接拿來用)
        // (在這個模擬中並沒有實際的去計算值，只是模擬他們的延遲，所以沒差)
